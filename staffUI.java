...
public class staffUI extends JFrame implements ActionListener{
    ...
    staffUI(String stfid){
        ...
        p4.setLayout(new GridLayout(5,1,0,10));
        p4.setBounds(80,140,300,200);
        p4.add(cat1);
        p4.add(add);
        p4.add(update);
        p4.add(issue);
	...
	public void actionPerformed(ActionEvent e){
        ...
        if(btn =="Update Book"){
            dispose();
			new updateBook(staffId);
        }
        if(btn =="Issue Book"){
            dispose();
			new issueBook(staffId);
        }
        if(btn =="Return Book"){
            dispose();
			new returnBook(staffId);
        }
        if(btn =="Add Member"){
            dispose();
			new addMember(staffId);
        }
        if(btn =="Update Member"){
            dispose();
			new updateMember(staffId);
        }
        if(btn =="Search Member"){
            dispose();
			new searchMember(staffId);
        }
        if(btn =="Logout"){
            dispose();
			new home();
